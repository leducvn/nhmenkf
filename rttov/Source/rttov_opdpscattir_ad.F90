...
!     1.9         14/12/2010   Use traj0_sta%sun array to flag channels for which solar calculations
...
!       Local scalars:
  ...
  REAL   (KIND=jprb) :: deltapice_ad
  REAL   (KIND=jprb) :: deltapice_d
  REAL   (KIND=jprb) :: deltapaer_ad
...
!-----End of header-----------------------------------------------------------------------
  ...
  PHASH            = 0._jprb
  phasice          = 0._jprb
  deltapice_ad     = 0._jprb
  deltapaer_ad     = 0._jprb
  opdpaerl(:,:)    = 0._jprb
  opdpaerlsun(:,:) = 0._jprb
  opdpcldl(:,:)    = 0._jprb
  opdpcldlsun(:,:) = 0._jprb
    ...
    DO ist = ircld%nstream(ipf), 0,  - 1
      IF (ist == 0) THEN
        IF (opts%addaerosl) THEN
          DO lay = nlayers, 1,  - 1
            lev = lay + 1
            opd = opd + transmission_scatt_ir_stream_ad%opdpac(IST, J, lev)
            transmission_scatt_ir_stream_ad%opdpacl(ist, j, lay) =      &
              & transmission_scatt_ir_stream_ad%opdpacl(ist, j, lay) + opd
            opdpaerl(j, lay)                                     =      &
              & opdpaerl(j, lay) + transmission_scatt_ir_stream_ad%opdpacl(ist, j, lay)
            IF (sun(j)) THEN
              opdsun = opdsun + transmission_scatt_ir_stream_ad%opdpacsun(ist, j, lev)
              transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay) =      &
                & transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay) + opdsun
              opdpaerlsun(j, lay)                                     =      &
                & opdpaerlsun(j, lay) + transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay)
              transmission_scatt_ir_ad%opdpsaer(j, lay)               = transmission_scatt_ir_ad%opdpsaer(j, lay) +      &
                & transmission_scatt_ir_stream_ad%opdpsca(ist, j, lay) *                                                 &
                & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ich)
              raytracing_ad%pathsat(lay, ipf)                         = raytracing_ad%pathsat(lay, ipf) +          &
                & transmission_scatt_ir_stream_ad%opdpsca(ist, j, lay) * transmission_scatt_ir%opdpsaer(j, lay) *  &
                & coef%ff_gam(ich)
              raytracing_ad%pathsun(lay, ipf)                         = raytracing_ad%pathsun(lay, ipf) +          &
                & transmission_scatt_ir_stream_ad%opdpsca(ist, j, lay) * transmission_scatt_ir%opdpsaer(j, lay) *  &
                & coef%ff_gam(ich)
              transmission_scatt_ir_ad%opdpaaer(j, lay)               = transmission_scatt_ir_ad%opdpaaer(j, lay) +      &
                & transmission_scatt_ir_stream_ad%opdpabs(ist, j, lay) *                                                 &
                & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ich)
              raytracing_ad%pathsat(lay, ipf)                         = raytracing_ad%pathsat(lay, ipf) +          &
                & transmission_scatt_ir_stream_ad%opdpabs(ist, j, lay) * transmission_scatt_ir%opdpaaer(j, lay) *  &
                & coef%ff_gam(ich)
              raytracing_ad%pathsun(lay, ipf)                         = raytracing_ad%pathsun(lay, ipf) +          &
                & transmission_scatt_ir_stream_ad%opdpabs(ist, j, lay) * transmission_scatt_ir%opdpaaer(j, lay) *  &
                & coef%ff_gam(ich)
              transmission_scatt_ir_ad%azphaerdoa(j, lay)             =      &
                & transmission_scatt_ir_ad%azphaerdoa(j, lay) + transmission_scatt_ir_stream_ad%azphacdo(ist, j, lay)
              transmission_scatt_ir_ad%azphaerupa(j, lay)             =      &
                & transmission_scatt_ir_ad%azphaerupa(j, lay) + transmission_scatt_ir_stream_ad%azphacup(ist, j, lay)
              transmission_scatt_ir_ad%gparaer(j, lay)                =      &
                & transmission_scatt_ir_ad%gparaer(j, lay) + transmission_scatt_ir_stream_ad%bcksp(ist, j, lay)
            ENDIF
          ENDDO
          opd = 0._jprb
          opdsun = 0._jprb
          transmission_scatt_ir_stream_ad%opdpac(IST, J, 1)    = 0._jprb
          transmission_scatt_ir_stream_ad%opdpacsun(IST, J, 1) = 0._jprb
        ELSE IF ((.NOT. opts%addaerosl) .AND. (opts%addclouds)) THEN
          transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, :) = 0._jprb
          transmission_scatt_ir_stream_ad%OPDPACSUN(IST, J, :)  = 0._jprb
          transmission_scatt_ir_stream_ad%OPDPACL(IST, J, :)    = 0._jprb
          transmission_scatt_ir_stream_ad%opdpac(ist, j, :)     = 0._jprb
          transmission_scatt_ir_stream_ad%azphacup(ist, j, :)   = 0._jprb
          transmission_scatt_ir_stream_ad%azphacdo(ist, j, :)   = 0._jprb
        ENDIF
      ELSE! ist/=0
...
!                              transmission_scatt_ir_stream_ad%opdpaclsun(ist,j,lay)
            opdpcldl(j, lay)                                     =      &
              & opdpcldl(j, lay) + transmission_scatt_ir_stream_ad%opdpacl(ist, j, lay) * ircld%icldarr(ist, lay, ipf)
            opdpaerl(j, lay)                                     =      &
              & opdpaerl(j, lay) + transmission_scatt_ir_stream_ad%opdpacl(ist, j, lay)
            IF (sun(j)) THEN
              opdsun = opdsun + transmission_scatt_ir_stream_ad%opdpacsun(ist, j, lev)
              transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay) =      &
                & transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay) + opdsun
              opdpcldlsun(j, lay)                                     = opdpcldlsun(j, lay) +      &
                & transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay) * ircld%icldarr(ist, lay, ipf)
              opdpaerlsun(j, lay)                                     =      &
                & opdpaerlsun(j, lay) + transmission_scatt_ir_stream_ad%opdpaclsun(ist, j, lay)
              IF ((ircld%icldarr(ist, lay, ipf) * transmission_scatt_ir%opdps(j, lay) +      &
                & transmission_scatt_ir%opdpsaer(j, lay)) /= 0._jprb) THEN
                raytracing_ad%pathsun(lay, ipf)             = raytracing_ad%pathsun(lay, ipf) +                            &
                  & transmission_scatt_ir_stream_ad%OPDPSCA(IST, J, lay) * coef%ff_gam(ICH) * ircld%ICLDARR(IST, lay, ipf) &
                  &  * transmission_scatt_ir%OPDPS(J, lay)
                raytracing_ad%pathsun(lay, ipf)             = raytracing_ad%pathsun(lay, ipf) +      &
                  & transmission_scatt_ir_stream_ad%OPDPSCA(IST, J, lay) * coef%ff_gam(ICH) *        &
                  & transmission_scatt_ir%OPDPSAER(J, lay)
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) +      &
                  & transmission_scatt_ir_stream_ad%OPDPSCA(IST, J, lay) * ircld%ICLDARR(IST, lay, ipf) *   &
                  & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ICH)
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) +      &
                  & transmission_scatt_ir_stream_ad%OPDPSCA(IST, J, lay) *                                     &
                  & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ICH)
                ...
                raytracing_ad%pathsat(lay, ipf)             = raytracing_ad%pathsat(lay, ipf) +      &
                  ...
                  & transmission_scatt_ir%OPDPSAER(J, lay)
                raytracing_ad%pathsun(lay, ipf)             = raytracing_ad%pathsun(lay, ipf) +                            &
                  & transmission_scatt_ir_stream_ad%OPDPABS(IST, J, lay) * coef%ff_gam(ICH) * ircld%ICLDARR(IST, lay, ipf) &
                  &  * transmission_scatt_ir%OPDPA(J, lay)
                raytracing_ad%pathsun(lay, ipf)             = raytracing_ad%pathsun(lay, ipf) +      &
                  & transmission_scatt_ir_stream_ad%OPDPABS(IST, J, lay) * coef%ff_gam(ICH) *        &
                  & transmission_scatt_ir%OPDPAAER(J, lay)
                transmission_scatt_ir_ad%OPDPA(J, lay)      = transmission_scatt_ir_ad%OPDPA(J, lay) +      &
                  & transmission_scatt_ir_stream_ad%OPDPABS(IST, J, lay) * ircld%ICLDARR(IST, lay, ipf) *   &
                  & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ICH)
                transmission_scatt_ir_ad%OPDPAAER(J, lay)   = transmission_scatt_ir_ad%OPDPAAER(J, lay) +      &
                  & transmission_scatt_ir_stream_ad%OPDPABS(IST, J, lay) *                                     &
                  & (raytracing%pathsat(lay, ipf) + raytracing%pathsun(lay, ipf)) * coef%ff_gam(ICH)
                ...
                raytracing_ad%pathsat(lay, ipf)             = raytracing_ad%pathsat(lay, ipf) +      &
                  ...
                  & transmission_scatt_ir%OPDPAAER(J, lay)
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -      &
                  & transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay) * transmission_scatt_ir%GPAR(J, lay) *  &
                ...
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                  & transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay) * ircld%ICLDARR(IST, lay, ipf) *     &
                ...
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) +      &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                    &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%GPAR(J, lay)       = transmission_scatt_ir_ad%GPAR(J, lay) +                     &
                  & transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay) * transmission_scatt_ir%OPDPS(J, lay) *            &
                  & ircld%ICLDARR(IST, lay, ipf) / (ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +  &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -         &
                  & transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay) * transmission_scatt_ir%GPARAER(J, lay) *  &
                  & transmission_scatt_ir%OPDPSAER(J, lay) / (                                                    &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                          &
                  & transmission_scatt_ir%OPDPSAER(J, lay)) ** 2
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                ...
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) +          &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                           &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%GPARAER(J, lay)    = transmission_scatt_ir_ad%GPARAER(J, lay) +            &
                  & transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay) * transmission_scatt_ir%OPDPSAER(J, lay) / ( &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                            &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -           &
                  & transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) * transmission_scatt_ir%AZPHDO(J, lay) *  &
                  & transmission_scatt_ir%OPDPS(J, lay) * ircld%ICLDARR(IST, lay, ipf) / (                          &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                            &
                  & transmission_scatt_ir%OPDPSAER(J, lay)) ** 2
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                  & transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) * ircld%ICLDARR(IST, lay, ipf) *  &
                ...
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) +      &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                    &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%AZPHDO(J, lay)     = transmission_scatt_ir_ad%AZPHDO(J, lay) +                   &
                  & transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) * transmission_scatt_ir%OPDPS(J, lay) *         &
                  & ircld%ICLDARR(IST, lay, ipf) / (ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +  &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -               &
                  & transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) * transmission_scatt_ir%AZPHAERDOA(J, lay) *  &
                  & transmission_scatt_ir%OPDPSAER(J, lay) / (                                                          &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                                &
                  & transmission_scatt_ir%OPDPSAER(J, lay)) ** 2
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                ...
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) +                &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                                 &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%AZPHAERDOA(J, lay) = transmission_scatt_ir_ad%AZPHAERDOA(J, lay) +            &
                  & transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) * transmission_scatt_ir%OPDPSAER(J, lay) / ( &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                               &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -           &
                  & transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) * transmission_scatt_ir%AZPHUP(J, lay) *  &
                  & transmission_scatt_ir%OPDPS(J, lay) * ircld%ICLDARR(IST, lay, ipf) / (                          &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                            &
                  & transmission_scatt_ir%OPDPSAER(J, lay)) ** 2
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                  & transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) * ircld%ICLDARR(IST, lay, ipf) *  &
                ...
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) +      &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                    &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%AZPHUP(J, lay)     = transmission_scatt_ir_ad%AZPHUP(J, lay) +                   &
                  & transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) * transmission_scatt_ir%OPDPS(J, lay) *         &
                  & ircld%ICLDARR(IST, lay, ipf) / (ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +  &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) -               &
                  & transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) * transmission_scatt_ir%AZPHAERUPA(J, lay) *  &
                  & transmission_scatt_ir%OPDPSAER(J, lay) / (                                                          &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                                &
                  & transmission_scatt_ir%OPDPSAER(J, lay)) ** 2
                transmission_scatt_ir_ad%OPDPS(J, lay)      = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
                ...
                transmission_scatt_ir_ad%OPDPSAER(J, lay)   = transmission_scatt_ir_ad%OPDPSAER(J, lay) +                &
                  ...
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                                 &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
                transmission_scatt_ir_ad%AZPHAERUPA(J, lay) = transmission_scatt_ir_ad%AZPHAERUPA(J, lay) +            &
                  & transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) * transmission_scatt_ir%OPDPSAER(J, lay) / ( &
                  & ircld%ICLDARR(IST, lay, ipf) * transmission_scatt_ir%OPDPS(J, lay) +                               &
                  & transmission_scatt_ir%OPDPSAER(J, lay))
              ELSE
                transmission_scatt_ir_stream_ad%AZPHACUP(IST, J, lay) = 0._jprb
                transmission_scatt_ir_stream_ad%AZPHACDO(IST, J, lay) = 0._jprb
                transmission_scatt_ir_stream_ad%BCKSP(IST, J, lay)    = 0._jprb
                transmission_scatt_ir_stream_ad%OPDPSCA(IST, J, lay)  = 0._jprb
                transmission_scatt_ir_stream_ad%OPDPABS(IST, J, lay)  = 0._jprb
              ENDIF
            ENDIF
          ENDDO
          opd = 0._jprb
...
!---------------Compute cloud  optical parameters ----------------------------------------
          transmission_scatt_ir_ad%OPDPCLDLA(J, lay) = transmission_scatt_ir_ad%OPDPCLDLA(J, lay) +      &
          ...
          IF (transmission_scatt_ir%OPDPS(J, lay) /= 0._JPRB) THEN
            transmission_scatt_ir_ad%AZPHUPTOT(J, lay) = transmission_scatt_ir_ad%AZPHUPTOT(J, lay) +      &
              & transmission_scatt_ir_ad%AZPHUP(J, lay) / transmission_scatt_ir%OPDPS(J, lay)
            transmission_scatt_ir_ad%OPDPS(J, lay)     = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
              & transmission_scatt_ir_ad%AZPHUP(J, lay) * transmission_scatt_ir%AZPHUPTOT(J, lay) /    &
              & (transmission_scatt_ir%OPDPS(J, lay) ** 2)
            transmission_scatt_ir_ad%AZPHDOTOT(J, lay) = transmission_scatt_ir_ad%AZPHDOTOT(J, lay) +      &
              & transmission_scatt_ir_ad%AZPHDO(J, lay) / transmission_scatt_ir%OPDPS(J, lay)
            transmission_scatt_ir_ad%OPDPS(J, lay)     = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
              & transmission_scatt_ir_ad%AZPHDO(J, lay) * transmission_scatt_ir%AZPHDOTOT(J, lay) /    &
              & (transmission_scatt_ir%OPDPS(J, lay) ** 2)
          ENDIF
        ENDIF
        transmission_scatt_ir_ad%OPDPCLDLA(J, lay) =      &
          & transmission_scatt_ir_ad%OPDPCLDLA(J, lay) + OPDPCLDL(J, lay) * raytracing%pathsat(lay, ipf) * coef%ff_gam(ICH)
        raytracing_ad%pathsat(lay, ipf)            =      &
          & raytracing_ad%pathsat(lay, ipf) + OPDPCLDL(J, lay) * transmission_scatt_ir%OPDPCLDLA(J, lay) * coef%ff_gam(ICH)
        transmission_scatt_ir_ad%OPDPA(J, lay)     =      &
          & transmission_scatt_ir_ad%OPDPA(J, lay) + transmission_scatt_ir_ad%OPDPCLDLA(J, lay)
        transmission_scatt_ir_ad%OPDPS(J, lay)     = transmission_scatt_ir_ad%OPDPS(J, lay) +      &
          & transmission_scatt_ir_ad%OPDPCLDLA(J, lay) * transmission_scatt_ir%GPAR(J, lay)
        transmission_scatt_ir_ad%GPAR(J, lay)      = transmission_scatt_ir_ad%GPAR(J, lay) +      &
          & transmission_scatt_ir_ad%OPDPCLDLA(J, lay) * transmission_scatt_ir%OPDPS(J, lay)
        IF (transmission_scatt_ir%OPDPS(J, lay) /= 0._JPRB) THEN
          transmission_scatt_ir_ad%GPARTOT(J, lay) = transmission_scatt_ir_ad%GPARTOT(J, lay) +      &
            & transmission_scatt_ir_ad%GPAR(J, lay) / transmission_scatt_ir%OPDPS(J, lay)
          transmission_scatt_ir_ad%OPDPS(J, lay)   = transmission_scatt_ir_ad%OPDPS(J, lay) -      &
            & transmission_scatt_ir_ad%GPAR(J, lay) * transmission_scatt_ir%GPARTOT(J, lay) /      &
            & (transmission_scatt_ir%OPDPS(J, lay) ** 2)
        ENDIF
!              transmission_scatt_ir_ad%GPAR (J,lay)    = 0._jprb
!              profiles_ad(IPF)%cloud(Ityp,lay)= profiles_ad(IPF)%cloud(Ityp,lay)       + &
!                          OPDPE(J,lay)         * &
!                          CONFAC(ICONF)                            * &
...
!-----------------Average phase function for the downward scattered solar beam------------
                ...
                transmission_scatt_ir_ad%AZPHDOCLS(J, lay, ityp) = transmission_scatt_ir_ad%AZPHDOCLS(J, lay, ityp) +      &
                  & transmission_scatt_ir_ad%AZPHDOTOT(J, lay) * transmission_scatt_ir%OPDPSCLS(J, lay, ityp)
                transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp)  = transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) +      &
                  & transmission_scatt_ir_ad%AZPHDOTOT(J, lay) * transmission_scatt_ir%AZPHDOCLS(J, lay, ityp)
                loop4 : DO k = iend, 0,  - iang
                  scattangle_d =      &
                    & musat_d * musun_d + sqrt(1 - musat_d ** 2) * sqrt(1 - musun_d ** 2) * cos(k * deg2rad)
                  phasint      =      &
                    & phasint + transmission_scatt_ir_ad%azphdocls(j, lay, ityp) * iang * deg2rad / (2 * pi)
                  DO kk = coef_scatt_ir%fmv_icl_ph - 1, 1,  - 1
                    IF (scattangle_d >= cosan_iCL(kk + 1) .AND. scattangle_d <= cosan_iCL(kk)) THEN
                      ...
                      DELTAPice_ad    = DELTAPice_ad + phasint * (COSAN_icl(KK) - SCATTANGLE_d) / DELTAice
                      SCATTANGLE      = SCATTANGLE - phasint * DELTAPice_d / DELTAice
                      phasint         = 0._jprb
                      PHAsice(KK + 1) = PHAsice(KK + 1) + deltapice_ad
                      PHAsice(KK)     = PHAsice(KK) - deltapice_ad
                      deltapice_ad    = 0._jprb
                      EXIT
                    ENDIF
                  ENDDO
                ...
                ENDDO loop4
                raytracing_ad%pathsat(lay, ipf)                  =      &
                  & raytracing_ad%pathsat(lay, ipf) + MUSAT / raytracing%pathsat(lay, ipf) ** 2
                raytracing_ad%pathsun(lay, ipf)                  =      &
                  & raytracing_ad%pathsun(lay, ipf) + MUSUN / raytracing%pathsun(lay, ipf) ** 2
                MUSAT = 0._jprb
                MUSUN = 0._jprb
                PHASINT = 0._jprb
!-----------------Average phase function for the upward scattered solar beam--------------
                ...
                transmission_scatt_ir_ad%AZPHUPCLS(J, lay, ityp) = transmission_scatt_ir_ad%AZPHUPCLS(J, lay, ityp) +      &
                  & transmission_scatt_ir_ad%AZPHUPTOT(J, lay) * transmission_scatt_ir%OPDPSCLS(J, lay, ityp)
                transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp)  = transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) +      &
                  & transmission_scatt_ir_ad%AZPHUPTOT(J, lay) * transmission_scatt_ir%AZPHUPCLS(J, lay, ityp)
                loop3 : DO k = iend, 0,  - iang
                  scattangle_d =      &
                    & musat_d * musun_d + sqrt(1 - musat_d ** 2) * sqrt(1 - musun_d ** 2) * cos(k * deg2rad)
                  phasint      =      &
                    & phasint + transmission_scatt_ir_ad%AZPHUPCLS(J, lay, ityp) * iang * deg2rad / (2 * pi)
                  DO kk = coef_scatt_ir%fmv_icl_ph - 1, 1,  - 1
                    IF (scattangle_d >= cosan_icl(kk + 1) .AND. scattangle_d <= cosan_icl(kk)) THEN
                      deltapice_d     = (PHAsice_d(KK + 1) - PHAsice_d(KK))
                      deltaice        = (COSAN_icl(KK) - COSAN_icl(KK + 1))
                ...
                DO k = 1, coef_scatt_ir%fmv_icl_comp - 1
                  IF (aux%dg(lay, ipf) >= coef_scatt_ir%fmv_icl_dg(k, ish) .AND.      &
                    ...
                    DO kk = 1, coef_scatt_ir%fmv_icl_ph
                      aux_ad%dg(lay, ipf) = aux_ad%dg(lay, ipf) + phasice(kk) * deltap(kk) / DELTAice
                      phasice(kk)         = 0._jprb
                    ENDDO
                    EXIT
                  ENDIF
...
!sun
            IF (ityp <= 5_jpim) THEN
              transmission_scatt_ir_ad%GPAR(J, lay)           = 0._jprb
              transmission_scatt_ir_ad%GPARCLS(J, lay, ityp)  = transmission_scatt_ir_ad%GPARCLS(J, lay, ityp) +      &
                & transmission_scatt_ir_ad%GPARTOT(J, lay) * transmission_scatt_ir%OPDPSCLS(J, lay, ityp)
              transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) = transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) +      &
                & transmission_scatt_ir_ad%GPARTOT(J, lay) * transmission_scatt_ir%GPARCLS(J, lay, ityp)
              transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) =      &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) + transmission_scatt_ir_ad%OPDPS(J, lay)
              profiles_ad(IPF)%cloud(Ityp, lay)               = profiles_ad(IPF)%cloud(Ityp, lay) +      &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * coef_scatt_ir%CONFAC(ICONF) *        &
                & optp%optpwcl(ityp)%sca(ich, 1) * raytracing%LTICK(nlayers + 1 - lay, IPF)
              raytracing_ad%LTICK(nlayers + 1 - lay, IPF)     = raytracing_ad%LTICK(nlayers + 1 - lay, IPF) +      &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * profiles(IPF)%cloud(Ityp, lay) *               &
                & coef_scatt_ir%CONFAC(ICONF) * optp%optpwcl(ityp)%sca(ich, 1)
              transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) =      &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) + transmission_scatt_ir_ad%OPDPA(J, lay)
              profiles_ad(IPF)%cloud(Ityp, lay)               = profiles_ad(IPF)%cloud(Ityp, lay) +      &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) * coef_scatt_ir%CONFAC(ICONF) *        &
                & optp%optpwcl(ityp)%abs(ich, 1) * raytracing%LTICK(nlayers + 1 - lay, IPF)
              raytracing_ad%LTICK(nlayers + 1 - lay, IPF)     = raytracing_ad%LTICK(nlayers + 1 - lay, IPF) +      &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) * profiles(IPF)%cloud(Ityp, lay) *               &
...
!-----------------------------------------------------------------------------------------
              ...
              transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) = transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) +      &
                & transmission_scatt_ir_ad%GPARTOT(J, lay) * transmission_scatt_ir%GPARCLS(J, lay, ityp)
              aux_ad%dg(lay, ipf)                             =      &
                & aux_ad%dg(lay, ipf) + transmission_scatt_ir_ad%GPARCLS(J, lay, ityp) * optp%optpicl(ish)%bpr(ich, 2)
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) +      &
                & transmission_scatt_ir_ad%GPARCLS(J, lay, ityp) * 2 * aux%dg(lay, ipf) * optp%optpicl(ish)%bpr(ich, 3)
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) +      &
                & transmission_scatt_ir_ad%GPARCLS(J, lay, ityp) * 3 * aux%dg(lay, ipf) ** 2 * optp%optpicl(ish)%bpr(ich, 4)
              transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) =      &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) + transmission_scatt_ir_ad%OPDPS(J, lay)
              raytracing_ad%LTICK(nlayers + 1 - lay, IPF)     = raytracing_ad%LTICK(nlayers + 1 - lay, IPF) +      &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * profiles(IPF)%cloud(Ityp, lay) * (             &
              ...
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) -                    &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * optp%optpicl(ish)%sca(ich, 4) *      &
                & profiles(IPF)%cloud(Ityp, lay) * raytracing%LTICK(nlayers + 1 - lay, IPF) * 2._jprb /  &
                & aux%dg(lay, ipf) ** 3
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) -                &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * optp%optpicl(ish)%sca(ich, 3) *  &
                & profiles(IPF)%cloud(Ityp, lay) * raytracing%LTICK(nlayers + 1 - lay, IPF) / aux%dg(lay, ipf) ** 2
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) +                &
                & transmission_scatt_ir_ad%OPDPSCLS(J, lay, ityp) * optp%optpicl(ish)%sca(ich, 2) *  &
              ...
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) -                    &
                & transmission_scatt_ir_ad%OPDPAcls(J, lay, ITYP) * optp%optpicl(ish)%abs(ich, 4) *      &
                & profiles(IPF)%cloud(Ityp, lay) * raytracing%LTICK(nlayers + 1 - lay, IPF) * 2._jprb /  &
                & aux%dg(lay, ipf) ** 3
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) -                &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) * optp%optpicl(ish)%abs(ich, 3) *  &
                & profiles(IPF)%cloud(Ityp, lay) * raytracing%LTICK(nlayers + 1 - lay, IPF) / aux%dg(lay, ipf) ** 2
              aux_ad%dg(lay, ipf)                             = aux_ad%dg(lay, ipf) +                &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) * optp%optpicl(ish)%abs(ich, 2) *  &
                & profiles(IPF)%cloud(Ityp, lay) * raytracing%LTICK(nlayers + 1 - lay, IPF)
              profiles_ad(IPF)%cloud(Ityp, lay)               = profiles_ad(IPF)%cloud(Ityp, lay) +                         &
                & transmission_scatt_ir_ad%OPDPACLS(J, lay, ityp) * (                                                       &
                & optp%optpicl(ish)%abs(ich, 1) + optp%optpicl(ish)%abs(ich, 2) * aux%dg(lay, ipf) +                        &
...
!cldtyp
        ENDDO
!ncldtyp
        transmission_scatt_ir_ad%azphup(j, lay) = 0._jprb
        transmission_scatt_ir_ad%azphdo(j, lay) = 0._jprb
        transmission_scatt_ir_ad%OPDPA(j, lay)  = 0._jprb
        transmission_scatt_ir_ad%OPDPS(j, lay)  = 0._jprb
        transmission_scatt_ir_ad%GPAR(j, lay)   = 0._jprb
!            transmission_scatt_ir_ad%OPDPAAER(j,lay)=0._jprb
!            transmission_scatt_ir_ad%OPDPSAER(j,lay)=0._jprb
!            transmission_scatt_ir_ad%GPARAER(j,lay) =0._jprb
      ENDDO
!nlayers
    ENDDO
!nchannels
...
!---------Compute final values for optical parameters-------------------------------------
      DO lay = nlayers, 1,  - 1
        lev = lay + 1
        IF (sun(j)) THEN
          transmission_scatt_ir_ad%OPDPAERLA(J, lay) = transmission_scatt_ir_ad%OPDPAERLA(J, lay) +      &
          ...
          raytracing_ad%pathsun(lay, ipf)            = raytracing_ad%pathsun(lay, ipf) +      &
            & OPDPAERLSUN(J, lay) * transmission_scatt_ir%OPDPAERLA(J, lay) * coef%ff_gam(ich)
        ENDIF
        transmission_scatt_ir_ad%OPDPAERLA(J, lay) =      &
          & transmission_scatt_ir_ad%OPDPAERLA(J, lay) + OPDPAERL(J, lay) * raytracing%pathsat(lay, ipf) * coef%ff_gam(ich)
        raytracing_ad%pathsat(lay, ipf)            =      &
          & raytracing_ad%pathsat(lay, ipf) + OPDPAERL(J, lay) * transmission_scatt_ir%OPDPAERLA(J, lay) * coef%ff_gam(ich)
        transmission_scatt_ir_ad%OPDPAAER(J, lay)  =      &
          & transmission_scatt_ir_ad%OPDPAAER(J, lay) + transmission_scatt_ir_ad%OPDPAERLA(J, lay)
        transmission_scatt_ir_ad%OPDPSAER(J, lay)  = transmission_scatt_ir_ad%OPDPSAER(J, lay) +      &
          & transmission_scatt_ir_ad%OPDPAERLA(J, lay) * transmission_scatt_ir%GPARAER(J, lay)
        transmission_scatt_ir_ad%GPARAER(J, lay)   = transmission_scatt_ir_ad%GPARAER(J, lay) +      &
          & transmission_scatt_ir_ad%OPDPAERLA(J, lay) * transmission_scatt_ir%OPDPSAER(J, lay)
        IF (transmission_scatt_ir%opdpsaer(j, lay) /= 0._jprb) THEN
          IF (sun(j)) THEN
            transmission_scatt_ir_ad%AZPHAERDO(J, lay) = transmission_scatt_ir_ad%AZPHAERDO(J, lay) +      &
              & transmission_scatt_ir_ad%AZPHAERDOA(J, lay) / transmission_scatt_ir%OPDPSAER(J, lay)
            transmission_scatt_ir_ad%OPDPSAER(J, lay)  = transmission_scatt_ir_ad%OPDPSAER(J, lay) -      &
              & transmission_scatt_ir_ad%AZPHAERDOA(J, lay) * transmission_scatt_ir%AZPHAERDO(J, lay) /   &
              & transmission_scatt_ir%OPDPSAER(J, lay) ** 2
            transmission_scatt_ir_ad%AZPHAERUP(J, lay) = transmission_scatt_ir_ad%AZPHAERUP(J, lay) +      &
              & transmission_scatt_ir_ad%AZPHAERUPA(J, lay) / transmission_scatt_ir%OPDPSAER(J, lay)
            transmission_scatt_ir_ad%OPDPSAER(J, lay)  = transmission_scatt_ir_ad%OPDPSAER(J, lay) -      &
              & transmission_scatt_ir_ad%AZPHAERUPA(J, lay) * transmission_scatt_ir%AZPHAERUP(J, lay) /   &
              & transmission_scatt_ir%OPDPSAER(J, lay) ** 2
          ENDIF
          transmission_scatt_ir_ad%GPARAERA(J, lay) = transmission_scatt_ir_ad%GPARAERA(J, lay) +      &
            & transmission_scatt_ir_ad%GPARAER(J, lay) / transmission_scatt_ir%OPDPSAER(J, lay)
          transmission_scatt_ir_ad%OPDPSAER(J, lay) = transmission_scatt_ir_ad%OPDPSAER(J, lay) -      &
            & transmission_scatt_ir_ad%GPARAER(J, lay) * transmission_scatt_ir%GPARAERA(J, lay) /      &
            & transmission_scatt_ir%OPDPSAER(J, lay) ** 2
        ENDIF
      ENDDO
      DO lay = nlayers, 1,  - 1
...
!-----------------Average phase function for the downward scattered solar beam------------
            ...
            musat_d =  - 1 / raytracing%pathsat(lay, ipf)
            musun_d =  - 1 / raytracing%pathsun(lay, ipf)
            profiles_ad(ipf)%aerosols(IAE, lay)         = profiles_ad(ipf)%aerosols(IAE, lay) +                         &
              & transmission_scatt_ir_ad%AZPHAERDO(J, lay) * transmission_scatt_ir%PHASINTDOREF(J, lay, I) * SCACH_D *  &
              & raytracing%LTICK(nlayers + 1 - lay, ipf)
            PHASINT = PHASINT +                                                                            &
              & transmission_scatt_ir_ad%AZPHAERDO(J, lay) * profiles(ipf)%aerosols(IAE, lay) * SCACH_D *  &
              & raytracing%LTICK(nlayers + 1 - lay, ipf)
            SCACH = SCACH + transmission_scatt_ir_ad%AZPHAERDO(J, lay) * profiles(ipf)%aerosols(IAE, lay) *      &
              & transmission_scatt_ir%PHASINTDOREF(J, lay, I) * raytracing%LTICK(nlayers + 1 - lay, ipf)
            raytracing_ad%ltick(nlayers + 1 - lay, ipf) = raytracing_ad%ltick(nlayers + 1 - lay, ipf) +      &
              & transmission_scatt_ir_ad%AZPHAERDO(J, lay) * SCACH_D * profiles(ipf)%aerosols(IAE, lay) *    &
            ...
            loop2 : DO K = iend, 0,  - iang
              ...
              DO kk = coef_scatt_ir%fmv_aer_ph - 1, 1,  - 1
                IF (scattangle_d >= cosan_aer(kk + 1) .AND. scattangle_d <= cosan_aer(kk)) THEN
                  ...
                  deltapaer_ad  = deltapaer_ad + phdo * (cosan_aer(kk) - scattangle_d) / deltaaer
                  scattangle    = scattangle - phdo * deltapaer_d / deltaaer
                  phdo          = 0._jprb
                  phash(kk + 1) = phash(kk + 1) + deltapaer_Ad
                  phash(kk)     = phash(kk) - deltapaer_ad
                  deltapaer_ad  = 0._jprb
                  EXIT
                ENDIF
              ENDDO
...
!-------------Average phase function for the upward scattered solar beam------------------
            ...
            musun_d =  - 1._jprb / raytracing%pathsun(lay, ipf)
            profiles_ad(ipf)%aerosols(IAE, lay)         = profiles_ad(ipf)%aerosols(IAE, lay) +                         &
              & transmission_scatt_ir_ad%AZPHAERUP(J, lay) * transmission_scatt_ir%PHASINTUPREF(J, lay, I) * SCACH_D *  &
              & raytracing%LTICK(nlayers + 1 - lay, ipf)
            PHASINT = PHASINT +                                                                            &
              & transmission_scatt_ir_ad%AZPHAERUP(J, lay) * profiles(ipf)%aerosols(IAE, lay) * SCACH_D *  &
              & raytracing%LTICK(nlayers + 1 - lay, ipf)
            SCACH = SCACH + transmission_scatt_ir_ad%AZPHAERUP(J, lay) * profiles(ipf)%aerosols(IAE, lay) *      &
              & transmission_scatt_ir%PHASINTUPREF(J, lay, I) * raytracing%LTICK(nlayers + 1 - lay, ipf)
            raytracing_ad%LTICK(nlayers + 1 - lay, ipf) = raytracing_ad%LTICK(nlayers + 1 - lay, ipf) +      &
              & transmission_scatt_ir_ad%AZPHAERUP(J, lay) * SCACH_D * profiles(ipf)%aerosols(IAE, lay) *    &
            ...
            loop1 : DO k = iend, 0,  - iang
              ...
              DO kk = coef_scatt_ir%fmv_aer_ph - 1, 1,  - 1
                IF (scattangle_d >= cosan_aer(kk + 1) .AND. scattangle_d <= cosan_aer(kk)) THEN
                  ...
                  deltapaer_ad  = deltapaer_ad + phup * (cosan_aer(kk) - scattangle_d) / deltaaer
...
!-----------------------------------------------------------------------------------------
          IF (i == 1) THEN
            profiles_ad(ipf)%aerosols(iae, lay)         = profiles_ad(ipf)%aerosols(iae, lay) +      &
              & transmission_scatt_ir_ad%GPARAERA(J, lay) * SCACH_D * BPARH_D * raytracing%ltick(nlayers + 1 - lay, ipf)
            SCACH = SCACH + transmission_scatt_ir_ad%GPARAERA(J, lay) * profiles(ipf)%aerosols(iae, lay) *      &
              & raytracing%ltick(nlayers + 1 - lay, ipf) * BPARH_D
            raytracing_ad%ltick(nlayers + 1 - lay, ipf) = raytracing_ad%ltick(nlayers + 1 - lay, ipf) +      &
              & transmission_scatt_ir_ad%GPARAERA(J, lay) * SCACH_D * profiles(ipf)%aerosols(iae, lay) * BPARH_D
            BPARH = BPARH + transmission_scatt_ir_ad%GPARAERA(J, lay) * profiles(ipf)%aerosols(iae, lay) *      &
              & raytracing%ltick(nlayers + 1 - lay, ipf) * SCACH_D
            profiles_ad(ipf)%aerosols(iae, lay)         = profiles_ad(ipf)%aerosols(iae, lay) +      &
              & transmission_scatt_ir_ad%OPDPSAER(J, lay) * SCACH_D * raytracing%ltick(nlayers + 1 - lay, ipf)
            SCACH = SCACH + transmission_scatt_ir_ad%OPDPSAER(J, lay) * profiles(ipf)%aerosols(iae, lay) *      &
              & raytracing%ltick(nlayers + 1 - lay, ipf)
            raytracing_ad%ltick(nlayers + 1 - lay, ipf) = raytracing_ad%ltick(nlayers + 1 - lay, ipf) +      &
              & transmission_scatt_ir_ad%OPDPSAER(J, lay) * SCACH_D * profiles(ipf)%aerosols(iae, lay)
            profiles_ad(ipf)%aerosols(iae, lay)         = profiles_ad(ipf)%aerosols(iae, lay) +      &
              & transmission_scatt_ir_ad%OPDPAAER(J, lay) * ABSCH_D * raytracing%ltick(nlayers + 1 - lay, ipf)
            ABSCH = ABSCH + transmission_scatt_ir_ad%OPDPAAER(J, lay) * profiles(ipf)%aerosols(iae, lay) *      &
              & raytracing%ltick(nlayers + 1 - lay, ipf)
            raytracing_ad%ltick(nlayers + 1 - lay, ipf) = raytracing_ad%ltick(nlayers + 1 - lay, ipf) +      &
              & transmission_scatt_ir_ad%OPDPAAER(J, lay) * ABSCH_D * profiles(ipf)%aerosols(iae, lay)
          ELSE
            profiles_ad(ipf)%aerosols(iae, lay)         = profiles_ad(ipf)%aerosols(iae, lay) +      &
              & transmission_scatt_ir_ad%GPARAERA(J, lay) * SCACH_D * BPARH_D * raytracing%ltick(nlayers + 1 - lay, ipf)
            SCACH = SCACH + transmission_scatt_ir_ad%GPARAERA(J, lay) * profiles(ipf)%aerosols(iae, lay) *      &
...
!               humidity.
            DO k = coef_scatt_ir%fmv_aer_rh(iae) - 1, 1,  - 1
              ...
              IF (sun(j)) THEN
                ...
                scach = 0._jprb
                bparh = 0._jprb
                aux_ad%relhum(lay, ipf) = aux_ad%relhum(lay, ipf) + frach
                frach = 0._jprb
                EXIT
              ENDIF
            ENDDO
    ...
    DO j = 1, nprofiles
      DO lay = nlayers, 1,  - 1
        ...
        ircld_ad%wmixave(lay, j) = ircld_ad%wmixave(lay, j) +                                      &
          & aux_ad%RELHUM(lay, j) * 100._jprb * 1e-6_jprb * 0.622_jprb * ircld%xpresave(lay, j) /  &
          & (ircld%PPV(lay, j) * (0.622_jprb + ircld%wmixave(lay, j) * 1e-6_jprb * 0.622_jprb))
        ircld_ad%wmixave(lay, j) = ircld_ad%wmixave(lay, j) -                                             &
          & aux_ad%RELHUM(lay, j) * 100._jprb * (1e-6_jprb * 0.622_jprb) ** 2 * ircld%xpresave(lay, j) *  &
        ...
        ircld_ad%PPV(lay, j)     = ircld_ad%PPV(lay, j) -                                                                  &
          & aux_ad%RELHUM(lay, j) * 100._jprb * ircld%wmixave(lay, j) * 1e-6_jprb * 0.622_jprb * ircld%xpresave(lay, j) *  &
          & (0.622_jprb + ircld%wmixave(lay, j) * 1e-6_jprb * 0.622_jprb) /                                                &
          & (ircld%PPV(lay, j) * (0.622_jprb + ircld%wmixave(lay, j) * 1e-6_jprb * 0.622_jprb)) ** 2
        IF(opts%lgradp) ircld_ad%xpresave(lay, j) = ircld_ad%xpresave(lay, j) + &
          &    100._jprb * ircld%wmixave(lay, j) * 1.e-6_jprb * 0.622_jprb * aux_ad%relhum(lay, j) /      &
...
!---------Compute vater vapour partial pressure-------------------------------------------
        ...
        ircld_ad%TAVE(lay, j) = ircld_ad%TAVE(lay, j) +      &
          & ircld_ad%ESI(lay, j) * ircld%ESI(lay, j) * 22.587_jprb * (T00 + 0.7_jprb) / (ircld%TAVE(lay, j) + 0.7_jprb) ** 2
!-----------------------------------------------------------------------------------------
      ENDDO
      DO lay = nlayers, 1,  - 1
        lev = lay + 1
        profiles_ad(j)%q(lev - 1) = profiles_ad(j)%q(lev - 1) + ircld_ad%wmixave(lay, j) / 2._jprb
        profiles_ad(j)%q(lev)     = profiles_ad(j)%q(lev) + ircld_ad%wmixave(lay, j) / 2._jprb
