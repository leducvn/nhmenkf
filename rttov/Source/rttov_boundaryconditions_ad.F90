  ...
  do ichan = 1, nchannels
    ...
    if( scatt_aux % cfrac (iprof) > ccthres .and. scatt_aux % mclayer (ichan) <= nlevels ) then 
      ...
      b (1)   = scatt_aux % ems_bnd(ichan) &
           & * (profiles (iprof) % skin % t - scatt_aux % b0 (iprof,nlevels)) &
...
!* ADJOINT PART
      ...
      !* Solve equations A * DX = B     
#ifdef _RTTOV_ECMWF   
      call dgbf   (tab, ldab, ndim, kl, ku, ipiv)                         
#else
...
#ifdef _RTTOV_ECMWF   
      call dgbs   (tab, ldab, ndim, kl, ku, ipiv, b_ad)    
#else
...
!      enddo
      ...
      scatt_aux_ad % bn (iprofad,mcly) = scatt_aux_ad % bn (iprofad,mcly) - b_ad (ndim)
      ...
      scatt_aux_ad % b0 (iprofad,nlevels) = scatt_aux_ad % b0 (iprofad,nlevels) &
        & - b_ad (1) * scatt_aux % ems_bnd (ichan)
  ...
  do ilayer=1,nlevels
    do ichan = 1, nchannels
      ...
      if( scatt_aux % cfrac (iprof) > ccthres .and. ilayer >= scatt_aux % mclayer(ichan)) then 
        ...
        scatt_aux_ad % b1 (iprofad,ilayer) = scatt_aux_ad % b1 (iprofad,ilayer) &
         & + bh_ad (ichan,ilayer) / scatt_aux % h  (ichan,ilayer)
        scatt_aux_ad % h  (ichan,ilayer)   = scatt_aux_ad % h  (ichan,ilayer)   &
         & - bh_ad (ichan,ilayer) * scatt_aux % b1 (iprof,ilayer) &
         & / scatt_aux % h (ichan,ilayer) / scatt_aux % h (ichan,ilayer)
