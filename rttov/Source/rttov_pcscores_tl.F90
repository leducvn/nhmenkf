  ...
  DO prof = 1, nprofiles
    DO i = 1, nchannels/nprofiles
      ...
      DO j = 1 + (prof-1) * npcscores/nprofiles, prof * npcscores/nprofiles
        ...
        pccomp_tl%pcscores(j) = pccomp_tl%pcscores(j) +      &
          & coef_pccomp%pcreg(opts%ipcreg)%coefficients(i, chan_pc) * radiancedata_tl%clear(k) / coef_pccomp%noise(chan)
      ENDDO
      k = k + 1
    ENDDO
  ENDDO
