...
!  1.3       18/08/2004  Fixed bug in K code (S English)
...
!  1.6       19/02/2007  Removed polarisation (R Saunders)
!  1.7       14/03/2007  Added abs for windratio (R Saunders)
!  1.8       23/10/2007  Now intialised einterpolated (R Saunders)
!  1.9       08/10/2008  Fixed adjoint/k bug for v < 0.0001; tidied (A Geer)
!  1.10      10/06/2009  Fixed strange bug related to initialisation of phi and u19 (R Saunders)
!
...
! Interpolate results to required frequency based on 7, 10, 19, 37 GHz
          ...
          zrough_v_k                        =  -                                                                     &
            & zreflmod_v_k * (transmission_aux%tau_surf(0, i) ** zrough_v * Log(transmission_aux%tau_surf(0, i))) /  &
            & (1.0_JPRB - transmission_aux%tau_surf(0, i))
          transmission_aux_k%tau_surf(0, i) = transmission_aux_k%tau_surf(0, i) + zreflmod_v_k * ( -                      &
          ...
          zrough_h_k                        =  -                                                                     &
            & zreflmod_h_k * (transmission_aux%tau_surf(0, i) ** zrough_h * Log(transmission_aux%tau_surf(0, i))) /  &
            & (1.0_JPRB - transmission_aux%tau_surf(0, i))
          transmission_aux_k%tau_surf(0, i) = transmission_aux_k%tau_surf(0, i) + zreflmod_h_k * ( -                      &
...
!Define nine predictors for the effective angle calculation
          ...
          zx_k(1) = 0._JPRB
!Compute surface to space optical depth
          transmission_aux_k%tau_surf(0, i) =      &
            & transmission_aux_k%tau_surf(0, i) - opdpsfc_k / (transmission_aux%tau_surf(0, i) * geom%seczen)
...
! relative azimuth angle in degree
        ...
        transmission_aux_k%tau_surf(0, i) = transmission_aux_k%tau_surf(0, i) + Transmittance_k
      END SELECT
      prof_k%skin%t = prof_k%skin%t + tcelsius_k
