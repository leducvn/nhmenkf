...
#include "rttov_errorreport.h"
  ...
  TYPE atlas_emis_mw
    ...
    REAL(KIND=jprb) :: lat1, lat2, lon1, lon2 
    ! Emissivities
    REAL(KIND=jprb), POINTER :: emis(:,:)!emis(ndat,nchan)
    ! Correlations
    REAL(KIND=jprb), POINTER :: correl(:,:,:)!correl(10,nchan,nchan)
    ! Emissivity uncertainties (std)
    REAL(KIND=jprb), POINTER :: emis_err(:,:)!emis_err(ndat,nchan)
    ! Surface classe
  ...
  TYPE(atlas_emis_mw), SAVE :: atlas
  
  ! User can specify atlas version at set-up. Version 100 is the default.
  INTEGER(KIND=jpim) :: mw_atlas_version=100   ! Version of atlas
...
CONTAINS
  ...
  SUBROUTINE test_inputs(month,lat,lon,theta,freq)
    ...
    INTEGER(KIND=jpim), INTENT (IN) :: month
    REAL(KIND=jprb), INTENT (IN)    :: lat, lon, theta, freq
  ...
  !------------------------------------------------------------------
  SUBROUTINE rttov_readmw_atlas(dir,month,err,lat1,lat2,lon1,lon2)
    !======Read a monthly atlas
    INTEGER(KIND=jpim), INTENT (IN)       :: month
    CHARACTER(len=*), INTENT (IN)         :: dir
    REAL(KIND=jprb),OPTIONAL, INTENT (IN) :: lat1, lat2, lon1, lon2 
    ...
    CHARACTER :: month2*2
    INTEGER   :: cellnum
    REAL(KIND=jprb) :: ssmi(2*7)
    REAL(KIND=jprb) :: lat,lon
    INTEGER   :: cur_class
    ...
    INFO('Reading number of data in atlas...')
  ...
  SUBROUTINE get_coordinates(cellnum,lat,lon)
    ...
    REAL(KIND=jprb), INTENT (OUT) :: lat, lon
    !INTEGER :: ilat,ilon
    INTEGER :: i
    REAL(KIND=jprb) :: res_lat ! latitude resolution
  ...
  SUBROUTINE calc_cellnum_mult(lat,lon,resol,cell_num_mult,nb_cell)
    ...
    REAL(KIND=jprb), INTENT(IN) :: resol
    INTEGER :: ilat,ilon, i
    INTEGER :: calc_cellnum
    !--------new variables
  ...
  SUBROUTINE emis_interp_ind_sing(lat,lon,theta,freq,ev,eh,stdv,stdh,covvh,verb)
    ...
    Implicit none     
    REAL(KIND=jprb), INTENT (IN)    :: lat, lon, theta, freq
    REAL(KIND=jprb), INTENT (OUT)   :: ev, eh   !resultat de l'interpolation
    REAL(KIND=jprb), OPTIONAL, INTENT (OUT) :: stdv, stdh, covvh
    INTEGER(KIND=jpim), INTENT (IN) :: verb
    INTEGER         :: ipos, i, j, i2, j2
    INTEGER         :: cellnum
    REAL(KIND=jprb) :: ev_a(3),eh_a(3)      !emissivity in the atlas
    ...
    cellnum=calc_cellnum(lat,modulo(lon,360.0_jprb))
  ...
  SUBROUTINE emis_interp_ind_mult(lat,lon,theta,freq,n_chan,ev,eh,std,verb)
    ...
    REAL(KIND=jprb), INTENT (IN)    :: lat, lon, theta, freq(:)
    REAL(KIND=jprb), INTENT (OUT)   :: ev(:), eh(:)   !resultat de l'interpolation
    REAL(KIND=jprb), OPTIONAL, INTENT (OUT) :: std(:,:)
    INTEGER(KIND=jpim), INTENT (IN) :: verb

    INTEGER         :: ipos, i, j, i2, j2
    INTEGER         :: cellnum
    REAL(KIND=jprb) :: ev_a(3),eh_a(3)      !emissivity in the atlas
    REAL(KIND=jprb) :: stdv_a(3),stdh_a(3)  !std in the atlas
    REAL(KIND=jprb) :: std2
    REAL(KIND=jprb) :: FIM(2*n_chan,3*2)    !Frequency Interpolation Matrix
    REAL(KIND=jprb) :: trans_std(3*2,2*n_chan)        !transpose Frequency Interpolation Matrix
    REAL(KIND=jprb) :: a,b,c                !frequency interpolation coefficients
    REAL(KIND=jprb) :: cov(6,6)             !covariance matrix of emis uncertainties in the atlas

    REAL(KIND=jprb) :: new_FIM(3*2,2*n_chan)          !transpose Frequency Interpolation Matrix
    REAL(KIND=jprb) :: correlation(2*n_chan,2*n_chan) !correlation
  ...
  SUBROUTINE emis_interp_int_sing(lat,lon,resol,theta,freq,ev,eh,stdv,stdh,covvh,verb)
    ...
    IMPLICIT none     
    REAL(KIND=jprb), INTENT (IN)            :: lat, lon, theta, freq
    REAL(KIND=jprb), INTENT (OUT)           :: ev, eh
    REAL(KIND=jprb), OPTIONAL, INTENT (OUT) :: stdv,stdh,covvh
    INTEGER(KIND=jpim), INTENT (IN)         :: verb
    REAL(KIND=jprb), INTENT (IN)            :: resol

    INTEGER          :: ipos, i, j, i2, j2
    REAL(KIND=jprb)  :: ev_a(3),eh_a(3)     !emissivities in the atlas
    INTEGER          :: ii
    INTEGER          :: nb_cell
    INTEGER, POINTER :: cellnum_mult(:) => NULL()
    REAL(KIND=jprb)  :: ev_mean, eh_mean
    INTEGER          :: inumb
    ...
    CALL calc_cellnum_mult(lat,modulo(lon,360.0_jprb),resol,cellnum_mult,nb_cell)
    ev_mean=0
    eh_mean=0
    std_mean(:,:)=0
    inumb=0
  ...
  SUBROUTINE emis_interp_int_mult(lat,lon,resol,theta,freq,n_chan,ev,eh,std,verb)
    ...
    INTEGER          :: ipos
    REAL(KIND=jprb)  :: ev_a(3),eh_a(3)     !emissivities in the atlas
    REAL(KIND=jprb)  :: stdv_a(3) !std emissivities in the atlas
    INTEGER          :: ii, i2, j2
    INTEGER          :: nb_cell
    INTEGER, POINTER :: cellnum_mult(:) => NULL()
    REAL(KIND=jprb)  :: ev_mean(n_chan), eh_mean(n_chan)
    REAL(KIND=jprb)  :: std_mean(2*n_chan,2*n_chan)
    REAL(KIND=jprb)  :: std2(2*n_chan,2*n_chan)
    REAL(KIND=jprb)  :: ev2, eh2
    INTEGER          :: inumb, i,j
  ...
  SUBROUTINE interp_freq2(emiss19,emiss37,emiss85,f,emiss,an,bn,cn)  
    ...
    IMPLICIT none
    REAL(KIND=jprb), INTENT (IN) :: emiss19,emiss37,emiss85,f
    REAL(KIND=jprb), INTENT (OUT) :: emiss
    REAL(KIND=jprb), OPTIONAL, INTENT (OUT) :: an, bn, cn
    REAL(KIND=jprb) :: a, b, c
    IF (f<=19.35) THEN
       a=1
       b=0
       c=0
 ...
 !------------------------------------------------------------------
   SUBROUTINE emis_interp(theta,freq,classe,ev,eh,emiss_interp_v,emiss_interp_h)
    ...
    REAL(KIND=jprb), INTENT (IN)  :: theta, freq, ev(3), eh(3)
    REAL(KIND=jprb), INTENT (OUT) :: emiss_interp_v, emiss_interp_h
    REAL(KIND=jprb) :: e0, theta0, theta53 , emiss_scal_v(3), emiss_scal_h(3)
    REAL(KIND=jprb) :: S1_v, S1_h, S2_v, S2_h, S_v, S_h, a0, a1, a2, a3, b0, b1
    REAL(KIND=jprb) :: b2, b3, em53_v, em53_h, emtheta_v, emtheta_h
    REAL(KIND=jprb) :: a0_k0(3,10),a0_k1(3,10),a0_k2(3,10)
    REAL(KIND=jprb) :: a0_eveh(3,10),a1_eveh(3,10),a2_eveh(3,10),a3_eveh(3,10)
    REAL(KIND=jprb) :: b0_eveh(3,10),b1_eveh(3,10),b2_eveh(3,10),b3_eveh(3,10)
    INTEGER         :: j
