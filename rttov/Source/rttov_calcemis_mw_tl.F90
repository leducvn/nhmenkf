...
!  1.6       09/02/2007  Removed  polarisation indexing (R Saunders)
...
!Loop over channels
  DO i = 1, nchannels
    ! CYCLE if emissivity_tl non-zero regardless of calcemis to allow 
    ! calculation of Jacobian using the TL in rttov_k_tl
    IF (.NOT. (calcemis(i) .AND. emissivity_tl(i) == 0.0_JPRB)) CYCLE
...
!Compute surface to space optical depth
          ...
          opdpsfc_tl  =  - transmission_aux_tl%tau_surf(0, i) / (transmission_aux%tau_surf(0, i) * geom%seczen)
...
!Switched h to v Deblonde SSMIS june 7, 2001
          ...
          zreflmod_v_tl          = transmission_aux_tl%tau_surf(0, i) * ( -                                               &
          ...
          zreflmod_v_tl          = zreflmod_v_tl -                                                                  &
            & (transmission_aux%tau_surf(0, i) ** zrough_v * Log(transmission_aux%tau_surf(0, i)) * zrough_v_tl) /  &
            & (1.0_JPRB - transmission_aux%tau_surf(0, i))
          zreflmod_h_tl          = transmission_aux_tl%tau_surf(0, i) * ( -                                               &
          ...
          zreflmod_h_tl          = zreflmod_h_tl -                                                                  &
            & (transmission_aux%tau_surf(0, i) ** zrough_h * Log(transmission_aux%tau_surf(0, i)) * zrough_h_tl) /  &
            & (1.0_JPRB - transmission_aux%tau_surf(0, i))
...
!           reflectstokes_tl(i,4)  =     -emissstokes_tl(i,4)
      ...
      CASE (4:5)
        ...
        Transmittance_tl = transmission_aux_tl%tau_surf(0, i)
