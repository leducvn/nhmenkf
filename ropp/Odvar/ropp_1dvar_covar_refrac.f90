! $Id: ropp_1dvar_covar_refrac.f90 4452 2015-01-29 14:42:02Z idculv $

!****s* Error covariances/ropp_1dvar_covar_refrac *
!
! NAME
...
!-------------------------------------------------------------------------------
! 1. Refractivity error covariances
!-------------------------------------------------------------------------------
...
! USE ropp_1dvar, not_this => ropp_1dvar_covar_refrac
  USE ropp_1dvar
  ...
  CALL message_set_routine('ropp_1dvar_covar_refrac')
...
!    ------------------------------------------------
     ...
     ELSE
        CALL message(msg_error, &
           "Refractivity error correlation / covariance file does not exist:\n")
...
! ------------------------------------
  ...
  CASE('FSFC')   ! Fixed sigmas and error correlations from error corr file

     IF (.NOT. ASSOCIATED(corcov(idx_lat)%sigma)) THEN
        CALL message(msg_error, &
             "Refractivity standard error deviations requested from " //    &
     ...
     IF (SIZE(obs % refrac) > n) THEN
        CALL message(msg_error, &
             "Not enough elements in the refractivity observation " //      &
             "error correlation file.\n")
        obs%cov_ok = .FALSE.
        RETURN
     ENDIF
     ...
     IF (SIZE(obs % refrac) < n) THEN
        CALL message(msg_warn, &
             "Too many elements in the refractivity observation " //        &
             "error correlation file.\n" //                                 &
...
! -----------------------------------------------
  ...
  CASE('VSDC')   ! Variable sigmas and diagonal error correlations
     ...
     IF (.NOT. obs % cov_ok) THEN
        CALL message(msg_error, & 
             "Refractivity standard error deviations requested on a " //    &
             "per profile basis,\n" //                                      &
             "   but observation data file did not contain sigmas.")
        obs%cov_ok = .FALSE.
        RETURN
     END IF
...
! ---------------------------------------
  ...
  CASE('VSFC')   ! Variable Sigmas from profile, 
                 ! Fixed Correlations from error correlation file
...
! ----------------------------------------------------
  ...
  CASE default
     ...
     CALL message(msg_error, &
          "Refractivity error covariance method " // &
