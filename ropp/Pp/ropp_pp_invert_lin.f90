! $Id: ropp_pp_invert_lin.f90 3551 2013-02-25 09:51:28Z idculv $

SUBROUTINE ropp_pp_invert_LIN(impact, bangle, nr, refrac, scale)

!****s* BendingAngle/ropp_pp_invert_LIN *
!
! NAME
...
!-------------------------------------------------------------------------------
  ...
  REAL(wp)                            :: delta_bangle
  REAL(wp)                            :: delta_impact
  REAL(wp)                            :: zt
  REAL(wp)                            :: erf_up  
...
!-------------------------------------------------------------------------------
  ...
  DO i = 1, n_impact - 1
     ...
     IF (bangle(i) < ropp_ZERO .OR. bangle(i+1) < ropp_ZERO) THEN
        kval(i) = 1.0e-12_wp
     ELSE
! Should probably use linear decay rate in this routine
!        kval(i) = LOG(bangle(i)/bangle(i+1))/MAX(1.0_wp,(impact(i+1)-impact(i)))
        kval(i) = (bangle(i) - bangle(i+1))/MAX(1.0_wp,(impact(i+1)-impact(i)))
        kval(i) = MAX(1.0e-12_wp, kval(i))
     ENDIF
  ENDDO
...
!    ----------------------------------
     ...
     i_bot = n_lower
     DO k = i_bot, n_impact - 1
        IF (nr(l) < impact(k + 1)) EXIT
     ENDDO
...
!       ----------------------------------------------------------------
        ...
        IF (bangle(i) <= ropp_MDTV .OR. bangle(i+1) <= ropp_MDTV) CYCLE

        IF (i == i_bot) THEN
! Should probably use linear decay rate in this routine
!           bangle_low = bangle(i_bot)*EXP(-kval(i_bot)*(nr(l) - impact(i_bot)))
           bangle_low = bangle(i_bot) - kval(i_bot)*(nr(l) - impact(i_bot))
           impact_low = nr(l)
        ELSE
           bangle_low = bangle(i) 
           impact_low = impact(i) 
...
!       ---------------------------------------
        ...
        delta_bangle = bangle(i+1) - bangle_low
        delta_impact = impact(i+1) - impact_low

        t_upper = SQRT((impact(i+1)-nr(l))*(impact(i+1)+nr(l)))
        t_lower = SQRT((impact_low-nr(l))*(impact_low+nr(l)))

        refrac(l) = refrac(l) +                                                &
          ((bangle_low*impact(i+1)-bangle(i+1)*impact_low)  &
             *LOG((t_upper+impact(i+1))/(t_lower+impact_low)) +                &
             delta_bangle*(t_upper-t_lower))/delta_impact

     ENDDO

     refrac(l) = refrac(l)/pi
...
!    -------------------------
     ...
     IF(PRESENT(scale))THEN
        ...
        t_upper = SQRT((impact(i_top)-nr(l))/scale)
        zt = 1.0_wp / (1.0_wp + 0.47047_wp * t_upper)
        erf_up = 1.0_wp - (a-(b-c*zt)*zt) * zt * EXP(-(t_upper*t_upper))

        refrac(l) = refrac(l) + &
            bangle(i_top)*EXP((impact(i_top)-nr(l))/scale)*    &
             (1.0_wp-erf_up)/ SQRT(pi*(impact(i_top)+nr(l))/scale)
