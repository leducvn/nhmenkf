...
! Calculate the coefficient of determination as well
   use variable
   use nhmlib
   use NodeInfo_class
   ...
   real(r_size) :: R0, ymean, ystd, ustd, rnorm0, ratio, norm, rmat1, rmat2
   ...
   call write_control(x, 1, info, 'xmean')
   do ie = 1, ne
   ...
   call power(x, 0.5d0)
   call write_control(x, 1, info, 'xstd')
   do ie = 1, ne
   ...
   call innerproduct(b, x, info, R0)
   call allreduce0D('xy', R0)
   !
   !
   !
   ! Project perturbations on b
   call innerproduct(b, b, info, norm)
   call allreduce0D('xy', norm)
   norm = sqrt(norm)
   call divide(b, norm)
   ustd = 0.d0
   do ie = 1, ne
      call innerproduct(xpert(ie), b, info, norm)
      call allreduce0D('xy', norm)
      ustd = ustd + norm**2
   end do
