...
c
c     blas daxpy2,dcopy2,ddot2,dswap
c     fortran dabs,iabs,mod
c
c     internal variables.
c
      double precision akkp1,ddot2,temp
...
c
               ...
               if (km1 .lt. 1) go to 170
                  call dcopy2(km1,ap(ik+1),1,work,1)
                  ij = 0
                  do 160 j = 1, km1
                     jk = ik + j
                     ap(jk) = ddot2(j,ap(ij+1),1,work,1)
                     call daxpy2(j-1,work(j),ap(ij+1),1,ap(ik+1),1)
                     ij = ij + j
  160             continue
                  ap(kk) = ap(kk) + ddot2(km1,work,1,ap(ik+1),1)
  170          continue
               kstep = 1
            go to 220
  180       continue
...
c
               ...
               if (km1 .lt. 1) go to 210
                  call dcopy2(km1,ap(ikp1+1),1,work,1)
                  ij = 0
                  do 190 j = 1, km1
                     jkp1 = ikp1 + j
                     ap(jkp1) = ddot2(j,ap(ij+1),1,work,1)
                     call daxpy2(j-1,work(j),ap(ij+1),1,ap(ikp1+1),1)
                     ij = ij + j
  190             continue
                  ap(kkp1+1) = ap(kkp1+1)
     *                         + ddot2(km1,work,1,ap(ikp1+1),1)
                  ap(kkp1) = ap(kkp1)
     *                       + ddot2(km1,ap(ik+1),1,ap(ikp1+1),1)
                  call dcopy2(km1,ap(ik+1),1,work,1)
                  ij = 0
                  do 200 j = 1, km1
                     jk = ik + j
...
c
            ...
            do 130 jj = 1, km1
               ...
               mulk = -ap(jk)/ap(kk)
               t = mulk
               call daxpy2(j,t,ap(ik+1),1,ap(ij+1),1)
               ijj = ij + j
               ap(jk) = mulk
               ij = ij - (j - 1)
  130       continue
...
c
            ...
            if (km2 .eq. 0) go to 180
               ...
               do 170 jj = 1, km2
                  ...
                  call daxpy2(j,t,ap(ik+1),1,ap(ij+1),1)
                  t = mulkm1
                  call daxpy2(j,t,ap(ikm1+1),1,ap(ij+1),1)
                  ap(jk) = mulk
                  ap(jkm1) = mulkm1
                  ijj = ij + j
                  ij = ij - (j - 1)
