program ldetkf1000
   ...
   !
   ! EigenExa
   call eigen_init(MPI_COMM_EWORLD)
   ...
   !do j = 1+djs, 1+djs
	 ...
	 call eigen_s(ne0, ne0, eigenA, eigennx, eigenW, eigenZ, eigennx, m_forward=32, m_backward=64)
	 YYT(:,:) = 0.
	 do eigen_jl = eigen_jstart, eigen_jend
	    eigen_jg = eigen_translate_l2g(eigen_jl, eigeny_nnod, eigeny_inod)
	    do eigen_il = eigen_istart, eigen_iend
	 ...
	 call allreduce2D('e', YYT, ne0, ne0) ! use U as V temporarily
	 !
	 ! Find rank
	 do ie = 1, ne0
	 ...
	 !if (myid == 0) print*, i, j, nrank, gammab(1), gammab(nrank)
	 ! V
	 ...
	 if (myid == 0) print*, i, j, mobs, nrank, lambda(1), d(1)
