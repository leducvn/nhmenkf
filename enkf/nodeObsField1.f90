...
! Created date: 13 Mar 2016
   ...
   interface loga10
      module procedure loga10_NodeObsField
   end interface
...
contains
   ...
   subroutine extract_y_NodeObsField2(self, i, j, ip, jp, obsspace, valid, mobs, y, np, mmax)
      ...
      integer, intent(inout) :: mobs
      real(r_size), dimension(mmax), intent(out) :: y
      integer :: k, ix, jy, it, iobs1, iobs2, iobs, jobs
   ...
   subroutine set_field_NodeObsField4(self, i, j, obsspace, object)
	 ...
	 iobs1 = obsspace%iobs(i,j,it)
	 iobs2 = obsspace%iobs(i,j,it) + obsspace%mobs(i,j,it) - 1
	 self%field(iobs1:iobs2,:) = object%field(iobs1:iobs2,:)
      end do
      !
      return
   ...
   subroutine add_obs_NodeObsField2(self, i, j, obsspace, object)
	 ...
	 iobs1 = obsspace%iobs(i,j,it)
	 iobs2 = obsspace%iobs(i,j,it) + obsspace%mobs(i,j,it) - 1
	 self%field(iobs1:iobs2,:) = self%field(iobs1:iobs2,:) + object%field(iobs1:iobs2,:)
      end do
      !
      return
   ...
   !
   !
   subroutine loga10_NodeObsField(self)
      implicit none
      type(NodeObsField), intent(inout) :: self
      !
      if (self%nobs == 0) return
      ...
      !
      return
   end subroutine loga10_NodeObsField
   !
   !
   !
   ...
   subroutine divide_obs_NodeObsField3(self, i, j, obsspace, const)
	 ...
	 iobs1 = obsspace%iobs(i,j,it)
	 iobs2 = obsspace%iobs(i,j,it) + obsspace%mobs(i,j,it) - 1
	 self%field(iobs1:iobs2,:) = self%field(iobs1:iobs2,:)/const
      end do
      !
      return
   ...
   subroutine divide_obs_NodeObsField4(self, i, j, obsspace, object)
	 ...
	 iobs1 = obsspace%iobs(i,j,it)
	 iobs2 = obsspace%iobs(i,j,it) + obsspace%mobs(i,j,it) - 1
	 where (object%field(iobs1:iobs2,:) < 1.e-12)
	    self%field(iobs1:iobs2,:) = 0.d0
	 else where
	    self%field(iobs1:iobs2,:) = self%field(iobs1:iobs2,:)/object%field(iobs1:iobs2,:)
	 end where
      end do
   ...
   !
   !
   subroutine qccheck_conv1(self, threshold, valid)
      implicit none
      type(NodeObsField), intent(in) :: self
	 ...
	 if (abs(self%field(iobs,1)) > threshold) then
      ...
      !
      return
   end subroutine qccheck_conv1
   !
   !
   !
   subroutine qccheck_conv2(self, threshold, i, j, obsspace, valid)
      implicit none
      type(NodeObsField), intent(in) :: self
      real(r_size), intent(in) :: threshold
      integer, intent(in) :: i, j
   ...
   !
   !
   subroutine qccheck_ra1(self, threshold, valid)
      implicit none
      type(NodeObsField), intent(in) :: self
      real(r_size), intent(in) :: threshold
      type(NodeObsValidField), intent(inout) :: valid
      ...
      do iobs = 1, self%nobs
	 if (abs(self%field(iobs,1)) > threshold) then
	 !if (abs(self%field(iobs,1)) > 6.d0) then
	    valid%field(iobs,1) = 0
	 else
	    valid%field(iobs,1) = 1
	 end if
   ...
   subroutine qccheck_NodeObsField1(self, threshold, valid)
	...
	& trim(self%name) == 'rvl') then
	 call qccheck_conv1(self, threshold, valid)
      else if (trim(self%name) == 'tc') then
	 call qccheck_tc1(self, threshold, valid)
      else if (trim(self%name) == 'ra') then
	 call qccheck_ra1(self, threshold, valid)
   ...
   subroutine qccheck_NodeObsField2(self, threshold, i, j, obsspace, valid)
	...
	& trim(self%name) == 'p' .or. trim(self%name) == 'rh' .or. trim(self%name) == 'pwv' .or. &
	& trim(self%name) == 'rvl') then
	 call qccheck_conv2(self, threshold, i, j, obsspace, valid)
      else if (trim(self%name) == 'tc') then
	 !call qccheck_tc2(self, threshold, valid)
      else if (trim(self%name) == 'ra') then
	 !call qccheck_ra2(self, threshold, valid)
      else if (trim(self%name) == 'rad') then
	 !call qccheck_rad2(self, threshold, valid)
      end if
      !
      return
   ...
   subroutine stdcheck_NodeObsField(self, valid)
      ...
      !
      do iobs = 1, self%nobs
	 if (self%field(iobs,1) < 1.e-12) then
	    valid%field(iobs,1) = 0
	 else
	    valid%field(iobs,1) = 1
	 end if
   ...
   subroutine random_obs_NodeObsField1(self)
      ...
      do iobs = 1, self%nobs
         do j = 1, self%nsubobs
            self%field(iobs,j) = 1.d0*random_normal()
         end do
      end do
      !
      return
   ...
   subroutine random_obs_NodeObsField2(self, valid)
      ...
      do iobs = 1, self%nobs
         do j = 1, self%nsubobs
            if (valid%field(iobs,j) == 1) then
	       self%field(iobs,j) = 1.d0*random_normal()
	    else
	       self%field(iobs,j) = 0.d0
	    end if
         end do
      end do
      !
