program envard
   ...
   !
   type(NodeInfo) :: info
   type(NodeControl) :: x, xbck, xobs, xobsbck, w0, std, u
   ...
   do irestart = 1, nrestart
	 ...
	 !call and(validtmp, valid)
      ...
      !if (myid == 0) print*, 'Rnorm:', 0, rnorm(0)
	 ...
	 !
	 ! Lanczos: convergence is slower than Arnoldi due to lost of orthogonality
	 !call innerproduct(q(iter), r, obsspace, valid, hmat(iter,iter))
	 !call allreduce0D(info, hmat(iter,iter))
	 !Az = q(iter)
	 !call multiply(Az, hmat(iter,iter))
	 !call subtract(r, Az)
	 !if (iter > 1) then
	    !hmat(1:iter-2,iter) = 0.d0
	    !hmat(iter-1,iter) = hmat(iter,iter-1)
	    !Az = q(iter-1)
	    !call multiply(Az, hmat(iter-1,iter))
	    !call subtract(r, Az)
	 !end if
	 !
	 ! Calculate new norm
	 ...
	 !if (myid == 0) print*, 'Rnorm:', iter, rnorm(iter)
	 ...
	 if (myid == 0) then
	    ...
	    ! Given rotation after Golub and Van Loan
	    !if (hmat(iter+1,iter) == 0.d0) then
	       !g(iter,1) = 1.d0
	       !g(iter,2) = 0.d0
	    !else if (abs(hmat(iter+1,iter)) > abs(rmat(iter,iter))) then
	       !rmat1 = -rmat(iter,iter)/hmat(iter+1,iter)
	       !g(iter,2) = 1.d0/sqrt(1.d0+rmat1**2)
	       !g(iter,1) = g(iter,2)*rmat1
	       !rmat(iter,iter) = abs(hmat(iter+1,iter))*sqrt(1.d0+rmat1**2)
	    !else
	       !rmat1 = -hmat(iter+1,iter)/rmat(iter,iter)
	       !g(iter,1) = 1.d0/sqrt(1.d0+rmat1**2)
	       !g(iter,2) = g(iter,1)*rmat1
	       !rmat(iter,iter) = abs(rmat(iter,iter))*sqrt(1.d0+rmat1**2)
	    !end if
	 ...
	 if (myid == 0) then
	    ...
	    write(90,*) istep, abs(p(iter+1)), ratio
	    close(90)
	 end if
