program mldetkf
   ...
   type(NodeProfileControl) :: xbck, xobs, zxpert, zxcontrol
   ...
   type(NodeObsControl), dimension(:), allocatable :: zypert
   type(NodeVModulation) :: vmod
   real(r_size), dimension(:,:), allocatable :: ftmp, htmp
   ...
   real(r_size) :: rhoML, rhoMLold, dh, lambdamean
   ...
   real(r_size), dimension(:,:), allocatable :: U, V, ZY, YYT
   real(r_size), dimension(:,:,:), allocatable :: ZYe
   !
   !
   !
   ! MPI Initialization
   ...
   call new_nhm_NodeProfileControl(zxpert, nxyt, nz0, nx, ny)
   ...
   do ie = 1, ne
	 ...
	 ! ZX
	 zxpert = xpert(ie)
	 call get_control(zxpert, xcontrol, zxcontrol)
	 call get_field(zxcontrol, ftmp, nlev)
	 call modulate(vmod, imode, ftmp, nxyt, nlev)
	 call set_field(zxcontrol, ftmp, nlev)
	 call set_control(zxpert, xcontrol, zxcontrol)
	 call apply_bc(bc, zxpert)
	 call apply_preH(preH, control_mode, processed(1,:,:), k2ijt, xbck, zxpert, xobs, nxyt)
   ...
   !if (myid == 191) allocate(htmp(nlev,ne))
   do ie = 1, ne
      do imode = 1, nmode
         iemode = (ie-1)*nmode + imode
	 ...
	 do it = 1, 2
	    ...
	    !if (myid == 191 .and. imode == 1 .and. it == 2) htmp(:,ie) = gtmp(1,1,:,1)
	    call modulate(vmod, imode, gtmp(:,:,:,1), xcontrol%nx, xcontrol%ny, nlev)
   ...
   deallocate(gtmp)
   !if (myid == 191) then
      !open(99,file='X.dat',form='binary')
      !write(99) htmp
      !close(99)
      !deallocate(htmp)
      !stop
   ...
   ! Allocate
   if (ncol0 > 0) then
      allocate(ZX(nemode), ZY(mmax,nemode), Y0(mmax))
      allocate(ZYe(mmax,nemode,nepe))
      allocate(YYT(nemode0,nemode0), gammab(nemode0), U(nemode0,nemode0), V(nemode,nemode0))
      allocate(d(nemode0), lambda(nemode0), flambda(nemode0), gammaa(nemode0), gammainfl(nemode0), VTX(nemode0))
   end if
   ...
   !do j = 1+djs, 1+djs
	 ...
	 do iemode = 1, nemode
	    call extract_y(zypert(iemode), i, j, ix(1:ncol), jy(1:ncol), loc.correlated(ivar,:), obsspace, valid, xyloc, mobs, ZY(:,iemode), ncol, mmax)
	 end do
	 call extract_y(obs, i, j, ix(1:ncol), jy(1:ncol), loc.correlated(ivar,:), obsspace, valid, xyloc, mobs, Y0, ncol, mmax)
	 if (mobs == 0) cycle
	 !
	 !
	 !
	 ...
	 if (mobs <= nemode0) then
	    ...
	    if (myide == 0) then
		  ...
		  YYT(ie,ie) = YYT(ie,ie) + 1.d0
	       ...
	       call mtx_eigen(1, nemode0, YYT, gammab, U, nrank) ! use U temporarily
	       gammab(:) = gammab(:) - 1.d0
	       ! Find rank
	       do ie = 1, nemode0
	 ...
	 !if (myid == 41) print*, myid, i, j, mobs, nrank, lambda(1), d(1)
	 ...
	 flambda(nrank+1:) = 1.d0
	 lambdamean = sum(flambda(:))/nemode0
	 rho%control(ivar)%p%field(ia,ja,1,1) = lambdamean
	 do iter = 1, rho%nvarnhm(ivar)
	    ...
	    do k = 1, nlevnhm
		  ...
		  VTX(ie) = sum(V(:,ie)*ZX(:))
	       ...
	       do ie = 1, ne
		  xapert(ie)%control(ivarnhm)%p%field(ia,ja,k,2) = lambdamean*xapert(ie)%control(ivarnhm)%p%field(ia,ja,k,2)
	       end do
	       !
	       if (itout == nt) cycle
	       ! Project X on V
