program envardpert1000
   ...
   do irestart = 1, nrestart
	 ...
	 if (irestart > 1) then
      ...
      !
      ! First norm: QR decomposition with modified Gram-Schmidt
      iter = 1
      beta(:,:) = 0.d0
      call set_index_nohalo(ne0, nepe, myide, k1, k2)
      do ie = 1, ne
      ...
      do ke = imember1, imember2
         ...
         call broadcast_ensobs(q(iter,ke), ide)
         call set_index_nohalo(ne0, nepe, myide, k1, k2)
         do ie = 1, ne
            je = ie+k1-1
            if (je < ke+1) cycle
      ...
      call allreduce2D('e', beta, imember2-imember1+1, imember2-imember1+1)
      p(:,:,:) = 0.d0
      p(1,:,:) = beta
      rnorm0 = sqrt(sum(beta**2))
      call MPI_BARRIER(MPI_COMM_WORLD, ierror)
      if (myid == 0) print*, 'Rnorm:', 0, rnorm0
      !
      !
      !
	 ...
	 ! Calculate new r
	 do ke = imember1, imember2
	    if (myid == 0) print*, 'Calculate r:', iter, ke
	    r(ke) = q(iter,ke)
	    ...
	    call apply_bcT(bc, x)
	    call MPI_BARRIER(MPI_COMM_WORLD, ierror)
	    !if (myid == 0) print*, 'Apply DHT:', iter, ke
	    !
	    ! C*w
	    if (weight_clim > 0.) then
	       call apply_USITclim(w0, std, x)
	    ...
	    if (weight_clim > 0.) call apply_USIclim(x, std, w0)
	    call MPI_BARRIER(MPI_COMM_WORLD, ierror)
	    !if (myid == 0) print*, 'Apply C:', iter, ke
	    call apply_bc(bc, x)
	    call apply_DpreH(preH, control_mode, x, xobs)
	    call update(xobs, info)
	    ...
	    call MPI_BARRIER(MPI_COMM_WORLD, ierror)
	    !if (myid == 0) print*, 'Apply DH:', iter, ke
	 end do
	 !
	 ! Block Arnoldi and Calculating new norm: QR decomposition with modified Gram-Schmidt
         hmat(1:iter+1,iter,:,:) = 0.d0
         call set_index_nohalo(ne0, nepe, myide, k1, k2)
         do ie = 1, ne
            ke = ie+k1-1
            q(iter+1,ke) = r(ke)
         ...
         do jter = 1, iter+1
            do ke = imember1, imember2
               if (jter == iter+1) then
                  ...
                  if (myide == ide) then
                     call innerproduct(q(jter,ke), q(jter,ke), obsspace, valid, hmat(jter,iter,ke,ke))
                     call allreduce0D('xy', hmat(jter,iter,ke,ke))
                     hmat(jter,iter,ke,ke) = sqrt(hmat(jter,iter,ke,ke))
                     call divide_obs(q(jter,ke), hmat(jter,iter,ke,ke))
                  end if
                  call broadcast_ensobs(q(jter,ke), ide)
               end if
               !
               call set_index_nohalo(ne0, nepe, myide, k1, k2)
               do ie = 1, ne
                  je = ie+k1-1
                  if (jter == iter+1 .and. je < ke+1) cycle
                  call innerproduct(q(jter,ke), q(iter+1,je), obsspace, valid, hmat(jter,iter,ke,je))
                  call allreduce0D('xy', hmat(jter,iter,ke,je))
                  Az = q(jter,ke)
                  call multiply_obs(Az, hmat(jter,iter,ke,je))
                  call subtract_obs(q(iter+1,je), Az)
               end do
            end do
         end do
         call allreduce3D('e', hmat(1:iter+1,iter,:,:), iter+1, imember2-imember1+1, imember2-imember1+1)
	 call MPI_BARRIER(MPI_COMM_WORLD, ierror)
	 !
	 if (myid == 0) then
	    if (myid == 0) print*, 'Apply QR:', iter
   ...
   do ke = imember1, imember2
      if (myid == 0) print*, 'Calculate r:', ke
   ...
   deallocate(rmata, rmatb, rmatc, rmatd, beta, p, s, hmat, rmat, g, ftmp)
   deallocate(vdz, vrdz, vrdz2, zrp, vctransp, dvtransp, zrw, vctransw, dvtransw, zs)
   call destroy(info)
   !
   call finalize_rttov()
   call MPI_FINALIZE(ierror)
end program envardpert1000
