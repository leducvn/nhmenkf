program mletkf
   ...
   !do j = 1+djs, 1+djs+10
	 ...
	 do ie = 1, ne
	    call extract_y(ypert(ie), i, j, ix(1:ncol), jy(1:ncol), loc.correlated(ivar,:), obsspace, valid, xyloc, mobs, Y(:,ie), ncol, mmax)
	 end do
	 call extract_y(obs, i, j, ix(1:ncol), jy(1:ncol), loc.correlated(ivar,:), obsspace, valid, xyloc, mobs, Y0, ncol, mmax)
	 !if (myide == 0 .and. mobs > nemode0) print*, myid, i, j, ncol, mmax, mobs
	 if (mobs == 0) cycle
	 !
	 !
	 !
	 ...
	 !if (myid == 42) print*, i, j, mobs, nrank, lambda(1), d(1)
